...
>(({ className, style, ...props }, ref) => {
  ...
  return (
    <DialogPrimitive.Overlay
      ...
      className={cn(
        'fixed inset-0 bg-[rgba(16,18,24,0.42)] dark:bg-black/60 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
        className,
      )}
      {...props}
    />
    ...
    return (
      <DialogPortal>
        ...
        <DialogPrimitive.Content
          ...
          className={cn(
// Width is w-11/12 below sm so the phone cap survives caller
            // `max-w-*` overrides — twMerge lets any caller max-w replace the
            // max-w cap, which used to make those dialogs full-bleed on phones.
            // Fill is --elevated per §7: this was on bg-background, which is
            // the page colour, so every dialog read as a hole in the page while
            // menus and the model picker read as surfaces on it.
            'fixed left-[50%] top-[50%] grid max-h-[90vh] w-full translate-x-[-50%] translate-y-[-50%] max-sm:!w-11/12 max-sm:!max-w-[91.666667%] gap-4 overflow-y-auto rounded-2xl bg-white dark:bg-surface-hover p-6 text-text-primary shadow-[0_16px_40px_rgba(16,18,24,0.14),0_4px_12px_rgba(16,18,24,0.07)] ring-1 ring-inset ring-border-light dark:shadow-[0_12px_34px_rgba(0,0,0,0.35)] dark:ring-0 duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]',
            className,
          )}
          {...props}
        >
          ...
          {showCloseButton && (
            <DialogPrimitive.Close
              /* §4 `.iconbtn`: a 32 box with a 16 glyph, no fill at rest. The
                 ring is `focus-visible` and neutral — it was `focus:ring-ring`,
                 which is the accent, so clicking the × with a mouse left a
                 purple ring sitting on a dialog that was already closing. */
              className="absolute right-4 top-4 grid size-8 place-items-center rounded-[8px] text-text-secondary-alt transition-colors hover:bg-surface-active hover:text-text-primary focus:outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-border-heavy disabled:pointer-events-none"
            >
