...
export const useGetModelsQuery = (
  ...
  return useQuery<t.TModelsConfig>([QueryKeys.models], () => dataService.getModels(), {
    ...
    // and counts as fresh, which combined with staleTime: Infinity means the
    // real /api/models fetch would never fire — callers used to paper over
    // that with refetchOnMount: 'always', refetching on every mount instead.
    // A placeholder renders immediately, the query still fetches exactly
    // once, and the response is then cached for the whole session.
    placeholderData: initialModelsConfig,
    refetchOnWindowFocus: false,
    refetchOnReconnect: false,
    refetchOnMount: false,
    staleTime: Infinity,
