...
export namespace Agents {
  ...
  export interface UserMessageMeta {
    ...
    text?: string;
    /** Files the user attached (e.g. an uploaded image) so a resume keeps them
     * visible in the chat alongside the prompt. */
    files?: unknown[];
  }

  /** A single generated image carried in a resume snapshot. */
  ...
  export interface ResumeState {
    ...
    conversationId?: string;
    sender?: string;
    /** Images already generated this turn — carried so a refresh mid-generation
     * doesn't drop images emitted before the reload. */
    generatedMedia?: GeneratedMediaItem[];
    /** Whether this turn is an image generation (recorded at request start) so
     * the "Generating image…" placeholder shows even before the first image
     * lands, independent of the input toggle (which a refresh may not restore). */
