...
export const tMessageSchema = z.object({
  ...
  /** Backboard Image Tool — images generated during this turn. */
  generatedMedia: z
    .array(
      z.object({
        documentId: z.string(),
    ...
    .optional(),
  /** True when this assistant turn is an image generation. Set on resume so the
   * "Generating image…" placeholder shows even before the first image lands,
   * independent of the input toggle (which a refresh may not restore). */
...
/** A single saved response (a message the user kept for later). */
export const tSavedMessageSchema = z.object({
  messageId: z.string(),
  conversationId: z.string(),
  /** `null` means the implicit "Unsorted" bucket. */
  folderId: z.string().nullable().optional(),
  text: z.string(),
...
export type TSavedMessage = z.infer<typeof tSavedMessageSchema>;

/** A folder that groups saved responses. The trailing `unsorted` row is virtual. */
export const tSavedMessageFolderSchema = z.object({
  folderId: z.string(),
  name: z.string(),
  description: z.string().optional(),
  createdAt: z.string(),
  updatedAt: z.string(),
  /** Newest member's timestamp; `''` when the folder is empty. List route only. */
  lastSavedAt: z.string().optional(),
  /** Present on the list route; omitted by the folder PATCH response. */
  savedCount: z.number().optional(),
  /** `true` only for the trailing implicit "Unsorted" row. */
  virtual: z.boolean().optional(),
});
export type TSavedMessageFolder = z.infer<typeof tSavedMessageFolderSchema>;
