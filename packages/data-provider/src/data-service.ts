...
export function continueSharedConvo(shareId: string): Promise<t.TForkConvoResponse> {
  return request.post(endpoints.continueSharedConvo(shareId), {});
}
...
export type AnalyticsScope = 'personal';
...
export interface AnalyticsModelRank {
  model: string;
  totalTokens: number;
}
...
export const apiKeyLogin = (payload: { apiKey: string }): Promise<t.TLoginResponse> => {
  return request.post(endpoints.apiKeyLogin(), payload);
};

export const storeByokKey = (payload: { provider: string; apiKey: string }): Promise<{
  session_key: string;
  session_token: string;
  provider: string;
  ttl_hours: number;
}> => {
  return request.post(endpoints.storeByokKey(), payload);
};

export const verifyByokSession = (payload: { session_key: string }): Promise<{
  valid: boolean;
  provider?: string;
}> => {
  return request.post(endpoints.verifyByokSession(), payload);
};

export const refreshByokSession = (payload: { session_key: string }): Promise<{
  message: string;
  ttl_hours: number;
}> => {
  return request.post(endpoints.refreshByokSession(), payload);
};
...
export const getMCPCatalog = (): Promise<{ catalog: MCPCatalogEntry[] }> => {
  return request.get(endpoints.mcpCatalog());
};

export const addMCPCatalogServer = (
  serverName: string,
): Promise<{ success: boolean; serverName: string; oauthRequired: boolean; oauthUrl: string }> => {
  return request.post(endpoints.mcpCatalogAdd(serverName));
};
...
/** Fraction uploaded, 0..1. Emitted only while the browser reports total bytes. */
export type UploadProgressHandler = (progress: number) => void;

function uploadConfig(signal?: AbortSignal | null, onProgress?: UploadProgressHandler) {
    ...
    ...(onProgress
      ? {
          onUploadProgress: (event: { loaded: number; total?: number }) => {
            // `total` is absent for chunked/streamed bodies; skip rather than
            // report a fabricated percentage.
...
export const getFilesUsage = (): Promise<t.TFileUsage> => {
  return request.get(endpoints.filesUsage());
};
...
export const uploadImage = (
  ...
  return request.postMultiPart(endpoints.images(), data, uploadConfig(signal, onProgress));
};
...
export const uploadFile = (
  ...
  return request.postMultiPart(endpoints.files(), data, uploadConfig(signal, onProgress));
};
...
export const exportFile = async ({
  ...
  return request.post(endpoints.fileExport(), { filename, content }, { responseType: 'blob' });
};
...
export function getSavedMessages(folderId?: string | null): Promise<t.TSavedMessagesResponse> {
  return request.get(endpoints.savedMessages(folderId));
}

export function saveMessage(payload: t.TSaveMessageRequest): Promise<s.TSavedMessage> {
  return request.post(endpoints.savedMessages(), payload);
}

export function updateSavedMessage(
  messageId: string,
  payload: t.TUpdateSavedMessageRequest,
): Promise<s.TSavedMessage> {
  return request.patch(endpoints.savedMessageById(messageId), payload);
}

export function deleteSavedMessage(messageId: string): Promise<{ message: string }> {
  return request.delete(endpoints.savedMessageById(messageId));
}

export function getSavedMessageFolders(): Promise<t.TSavedMessageFoldersResponse> {
  return request.get(endpoints.savedMessageFolders());
}

export function createSavedMessageFolder(
  payload: t.TSavedMessageFolderRequest,
): Promise<s.TSavedMessageFolder> {
  return request.post(endpoints.savedMessageFolders(), payload);
}

export function updateSavedMessageFolder(
  folderId: string,
  payload: t.TSavedMessageFolderRequest,
): Promise<s.TSavedMessageFolder> {
  return request.patch(endpoints.savedMessageFolderById(folderId), payload);
}

export function deleteSavedMessageFolder(
  folderId: string,
): Promise<t.TDeleteSavedFolderResponse> {
  return request.delete(endpoints.savedMessageFolderById(folderId));
}
