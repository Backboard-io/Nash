...
describe('StartupLayout — redirect race condition', () => {
  ...
  it('uses the router location instead of a later global URL when checking redirect_to', async () => {
    Object.defineProperty(window, 'location', {
      value: { ...originalLocation, search: '?token=invite-token' },
      writable: true,
    });

    const router = createTestRouter(
      '/login?redirect_to=%2Forganizations%2Finvite%3Ftoken%3Dinvite-token',
      true,
    );
    render(<RouterProvider router={router} />);
