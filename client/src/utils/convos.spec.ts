...
describe('date grouping matches the designer dates card', () => {
  ...
  it('keeps a pinned chat in its date group instead of a Pinned section', () => {
    ...
    expect(names).not.toContain('com_ui_pinned');
    const today = groups.find(([name]) => name === 'com_ui_date_today')?.[1] ?? [];
    expect(today.map((c: any) => c.conversationId)).toEqual(['a', 'c']);
    const june = groups.find(([name]) => name === 'com_ui_date_june')?.[1] ?? [];
    expect(june.map((c: any) => c.conversationId)).toEqual(['b']);
  });
});
