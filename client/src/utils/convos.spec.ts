...
describe('date grouping matches the designer dates card', () => {
  // Card sample (viewed in August): Today, Yesterday, Previous 7 days,
  // Previous 30 days, June, March, February — relative labels first, then
  // month names newest-first, then bare years for earlier years.
  const convo = (id: string, iso: string, isPinned = false) =>
    ({ conversationId: id, updatedAt: iso, isPinned }) as any;
  ...
  it('produces the card order even from scrambled input', () => {
    const scrambled = [
      convo('mar', '2026-03-10T10:00:00Z'),
      convo('feb', '2026-02-05T10:00:00Z'),
      convo('today', '2026-08-12T09:00:00Z'),
      convo('lastyear', '2025-12-20T10:00:00Z'),
      convo('jun', '2026-06-15T10:00:00Z'),
      convo('week', '2026-08-08T10:00:00Z'),
      convo('yesterday', '2026-08-11T15:00:00Z'),
      convo('month', '2026-07-20T10:00:00Z'),
    ];
    const groups = groupConversationsByDate(scrambled);
  ...
  it('collects pinned chats into a Pinned section ahead of the date groups', () => {
    const groups = groupConversationsByDate([
      convo('a', '2026-08-12T09:00:00Z', true),
      convo('b', '2026-06-15T10:00:00Z', true),
      convo('c', '2026-08-12T08:00:00Z'),
    ]);
    const names = groups.map(([name]) => name);
    // Pinned leads the list, newest first, and its chats leave their date group.
    expect(names[0]).toBe('Pinned');
    const pinned = groups.find(([name]) => name === 'Pinned')?.[1] ?? [];
    expect(pinned.map((c: any) => c.conversationId)).toEqual(['a', 'b']);
    const today = groups.find(([name]) => name === 'com_ui_date_today')?.[1] ?? [];
    expect(today.map((c: any) => c.conversationId)).toEqual(['c']);
    expect(names).not.toContain('com_ui_date_june');
  });
});
