...
export const groupConversationsByDate = (
  ...
  // Pinned chats leave their date group and collect under a "Pinned" marker
  // at the top of the list, as in the Figma sidebar. The pin is still shown
  // on the row itself — it is both a state and a section.
  ...
  conversations.forEach((conversation) => {
    ...
    if ((conversation as { isPinned?: boolean }).isPinned === true) {
  ...
  // Months (always the current year) sort newest-first and precede the bare
  // year groups, which sort descending. Explicit ranks: the previous version
  // parseInt'd month keys to NaN, which sorts as 0, so month order silently
  // depended on the API already returning conversations newest-first.
  const groupRank = (group: string): number => {
    const year = parseInt(group.trim(), 10);
    if (!Number.isNaN(year)) {
      return year;
    }
    const month = monthOrderMap.get(dateKeysReverse[group] ?? '') ?? -1;
    return 100000 + month; // any month of the current year outranks any bare year
  };
  const yearMonthGroups = Array.from(groups.keys())
    .filter((group) => !dateGroupsSet.has(group))
    .sort((a, b) => groupRank(b) - groupRank(a));
  ...
  const dated = Array.from(sortedGroups, ([key, value]) => [key, value]) as GroupedConversations;
  if (pinned.length === 0) {
    return dated;
  }
  pinned.sort(
  ...
  return [['Pinned', pinned], ...dated] as GroupedConversations;
};
