...
export default function useExportConversation({
  ...
  const getMessageContent = (sender: string, content?: TMessageContentParts): string[] => {
    ...
    if (content.type === ContentTypes.TEXT) {
      ...
      // Parts tagged with agentId are parallel (compare-mode) columns —
      // the message-level `sender` is the same generic string ("Nash") for
      // every column, so without this every model's answer would export
      // under an identical, indistinguishable label.
      const agentId = (content as { agentId?: string }).agentId;
      if (agentId != null) {
        const parsed = parseEphemeralAgentId(agentId);
        const label = parsed?.sender || parsed?.model || stripAgentIdSuffix(agentId) || sender;
        return [label, text];
      }
      return [sender, text];
    }
