...
import { derivePastedFilename } from '~/utils/pastedText';
...
export default function useChatFunctions({
  ...
  const fallbackModel = useRecoilValue(store.fallbackModel);
  const defaultImageModel = useRecoilValue(store.defaultImageModel);
  const perConvoImageModel = useRecoilValue(
    store.imageModelByConversation(immutableConversation?.conversationId ?? Constants.NEW_CONVO),
  );
  ...
  const ask: TAskFunction = (
    ...
    text = text.trim();
    const pastedBlockFiles: Partial<TFile>[] = pastedBlocks.map((block) => {
      const filename = derivePastedFilename(block.text, block.isCode, block.ext);
      return {
        file_id: `pasted-${block.id}`,
        filename,
        filepath: '',
        type: block.isCode ? `text/x-${block.ext || 'plain'}` : 'text/plain',
        bytes: new Blob([block.text]).size,
        embedded: true,
        object: 'file',
        usage: 0,
        ...
        metadata: {
          ...
          isPastedBlock: true,
          lineCount: block.text.replace(/\n$/, '').split('\n').length,
        },
      };
    });

    const hasPendingFiles = (files?.size ?? 0) > 0;
    const hasOverrideFiles = (overrideFiles?.length ?? 0) > 0;
    if (
      isSubmitting ||
      (text === '' &&
        pastedBlockFiles.length === 0 &&
        !hasPendingFiles &&
        !hasOverrideFiles)
    ) {
      return;
    ...
    // Per-conversation image model (composer picker) wins over the user-wide
    // Settings → Preferences default.
    const effectiveImageModel = perConvoImageModel || defaultImageModel;
    ...
    if (setFiles && reuseFiles === true) {
      currentMsg.files = [...submissionFiles, ...pastedBlockFiles];
      setFiles(new Map());
      setFilesToDelete({});
      ...
      currentMsg.files = [
        ...Array.from(files.values()).map((file) => ({
          ...
          // Carry the local blob preview so the attached image renders on the
          // optimistic user message while the server-side path is still being
          // processed.
      ...
      setFiles(new Map());
      setFilesToDelete({});
    } else if (pastedBlockFiles.length > 0) {
      currentMsg.files = pastedBlockFiles;
    }
