...
const useFileDeletion = ({
  ...
  const deleteFile = useCallback(
    ({ file: _file, setFiles }: { file: ExtendedFile | t.TFile; setFiles?: FileMapSetter }) => {
      ...
      // An in-flight or failed upload has nothing persisted server-side yet, so
      // there is no batch delete to queue — but it must still leave the
      // composer, otherwise the X appears dead and `filesLoading` keeps the
      // send button disabled with no way to clear it.
      if (progress < 1 || attached) {
        return;
      }
