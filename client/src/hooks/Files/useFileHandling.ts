...
const useFileHandling = (params?: UseFileHandling) => {
  ...
  /** Real per-file upload progress (0..1), keyed by temp file id. */
  const [uploadProgress, setUploadProgress] = useState<Record<string, number>>({});
  const dropProgress = useCallback((fileId?: string) => {
  ...
  const uploadFile = useUploadFileMutation(
    {
      onSuccess: (data) => {
        ...
        dropProgress(data.temp_file_id);
      ...
      onError: (_error, body) => {
        ...
        dropProgress(file_id as string);
    ...
    (fileId, progress) =>
      setUploadProgress((prev) =>
        prev[fileId] === progress ? prev : { ...prev, [fileId]: progress },
      ),
  );
