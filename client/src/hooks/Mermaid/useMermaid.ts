...
// Lazy load mermaid library (~2MB). The memo is reset on rejection so one
// transient chunk-load failure (deploy rotating the hashed chunk, network
// blip) doesn't leave diagram rendering broken for the whole session.
