...
export default function useEventHandlers({
  ...
  const createdHandler = useCallback(
    (data: TResData, submission: EventSubmission) => {
      ...
      // Commit the URL to the real conversation id as soon as it's known (the
      // `created` event), not only at the final event. Otherwise a slow
      // generation (e.g. image generation) leaves the URL at /c/new, and a
      // refresh mid-stream can't resume the still-running server stream —
      // useResumeOnLoad short-circuits on NEW_CONVO — so the thread appears
      // lost. Guarded to a genuinely new, non-temporary conversation still
      // sitting on /c/new. (finalHandler does the same navigate at completion.)
      // Read the browser path at event time because the SSE listener can retain
      // this handler across router renders, making useLocation's snapshot stale.
      if (
        !isTemporary &&
        !isAddedRequest &&
        conversationId &&
        conversationId !== Constants.NEW_CONVO &&
        window.location.pathname === `/c/${Constants.NEW_CONVO}`
      ) {
        // Commit the URL to the real id WITHOUT a React-Router navigation. A real
        // navigate here remounts ChatView mid-stream and blanks the just-sent
        // image + prompt until the response completes — the chat must "stay how
        // it is" while streaming. history.replaceState only updates the address
        // bar, so the view stays mounted on /c/new and its messages keep
        // streaming in place, while a refresh still lands on /c/<id> and resumes
        // (useResumeOnLoad reads the URL). The real React-Router navigation
        // happens in finalHandler at completion — seamless there because it
        // seeds the id's message cache before navigating, and generation is done
        // so there's no live view to disrupt.
        try {
          window.history.replaceState(window.history.state, '', `/c/${conversationId}`);
        } catch {
          // history API unavailable — fall back to a real navigation.
  ...
  const finalHandler = useCallback(
    (data: TFinalResData, submission: EventSubmission) => {
      ...
      logger.debug('nash:finalHandler', 'enter', {
      ...
      try {
        ...
        if ((data as Record<string, unknown>).earlyAbort) {
          logger.debug('nash:finalHandler', 'early abort before saved messages');
        ...
        if (!currentMessages || currentMessages.length === 0) {
          logger.debug('nash:finalHandler', 'no current messages, returning early');
          return;
        }
        ...
        if (isNewConvo && finalConversationId && !_isTemporary) {
         ...
         * Temporary chats are never listed. */
        if (isNewConvo && finalConversationId && finalConversation && !_isTemporary) {
        ...
        if (setConversation && isAddedRequest !== true) {
          ...
          /* Temporary chats never navigate to /c/<id>: the conversation is not
           * fetchable server-side (nothing persisted), so that URL would show a
           * permanent loading state. They live on /c/new for their whole life —
           * in-memory only, gone on refresh (ChatGPT semantics). */
          if (
            window.location.pathname === `/c/${Constants.NEW_CONVO}` &&
            finalConversationId &&
            !_isTemporary
        ...
        logger.debug('nash:finalHandler', 'applied final state', {
          finalConversationId,
          messageConvoId,
        ...
        logger.debug('nash:finalHandler', 'cleanup', {
          submissionConversationId: submissionConvo.conversationId,
        });
        setSubmission?.(null);
        setShowStopButton(false);
  ...
  const errorHandler = useCallback(
    ({ data, submission }: { data?: TResData; submission: EventSubmission }) => {
      ...
      logger.debug('nash:errorHandler', 'enter', {
