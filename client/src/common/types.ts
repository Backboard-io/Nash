...
export type TShowToast = {
  ...
  /** Mirrors @librechat/client's TShowToast — a single inline action, used
   *  for the Undo that DESIGN.md §5 requires after a destructive step. */
...
export type TPastedBlockPayload = {
  id: string;
  text: string;
  language: string;
  ext: string;
...
export type TOptions = {
  ...
  pastedBlocks?: TPastedBlockPayload[];
...
export type MentionOption = OptionWithIcon & {
  ...
  value: string;
  description?: string;
  /** Raw model id for model mentions; `label` is the display name only */
  model?: string;
};
...
export type TFinalResData = Omit<TBaseResData, 'conversation'> & {
  conversation?: Partial<t.TConversation> | null;
  requestMessage?: t.TMessage;
  responseMessage?: t.TMessage;
};
