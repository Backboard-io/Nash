...
import { countLines } from '~/utils/pastedText';
...
function PastedCodeCard({ block, onRemove }: { block: PastedBlock; onRemove: () => void }) {
  ...
  const label = block.isCode ? 'Pasted code' : 'Pasted text';
  ...
  return (
    <div className="group relative w-72 max-w-full overflow-hidden rounded-2xl border border-border-light bg-surface-hover-alt text-sm text-text-primary">
      <div className="flex items-center gap-2 p-2">
        <div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-[#635BFF]/10 text-[#4F48D9] dark:text-[#B9B5FF]">
        ...
        <button
          ...
          className="rounded-md p-1 text-text-secondary transition-colors hover:bg-surface-hover"
        >
          <ChevronDown
