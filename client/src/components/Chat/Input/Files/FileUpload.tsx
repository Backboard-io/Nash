...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    reader.onload = (e) => {
      ...
      if (validator) {
        setStatus('success');
        setStatusColor('text-brand-purple dark:text-brand-purple');
      }
  ...
  return (
    <>
      <button
        ...
        onClick={handleClick}
        className={cn(
          /* Hover brightens to --t1 on a --hover fill. This had a light-mode grey
             pair, a dark-mode grey pair and accent text in both — three
             hard-coded greys where the ladder has one token each way. */
          'mr-1 flex h-auto cursor-pointer items-center rounded bg-transparent px-2 py-1 text-xs font-normal text-text-secondary transition-colors hover:bg-surface-hover hover:text-text-primary focus:ring-ring',
          statusColor,
          containerClassName,
        )}
