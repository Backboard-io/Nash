...
const FilePreview = ({
  ...
  const isPastedCodeFile =
    metadata?.isPastedBlock === true &&
    file?.type?.startsWith('text/x-') === true &&
    metadata?.displayLanguage !== 'Text';
  const isCodeFile = !!metadata?.fileIdentifier || isPastedCodeFile;

  return (
    <div className={cn('relative size-10 shrink-0 overflow-hidden rounded-xl', className)}>
      <FileIcon file={file} fileType={fileType} />
      <SourceIcon source={file?.source} isCodeFile={isCodeFile} />
