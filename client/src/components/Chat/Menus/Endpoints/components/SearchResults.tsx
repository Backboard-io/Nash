  ...
  if (!results.length) {
    return (
      <>
        ...
        <div className="cursor-default px-[10px] py-2 text-[12.5px] leading-[18.75px] text-text-secondary">
          {localize('com_files_no_results')}
        </div>
      </>
    );
        ...
        if ('name' in item && 'label' in item) {
          ...
          return (
            <MenuItem
              ...
              className={cn(
                'flex min-h-9 w-full cursor-pointer justify-between text-[13px] leading-[19.5px]',
          ...
          if (endpoint.hasModels && endpoint.models && endpoint.models.length > 0) {
            ...
            return (
              <Fragment key={`endpoint-${endpoint.value}-search-${i}`}>
                <div className="flex items-center gap-2 px-[10px] py-1 text-[12px] font-medium leading-[18px] text-text-secondary-alt">
                  {endpoint.label}
                </div>
                {filteredModels.map((model) => {
                  ...
                  let isGlobal = false;
                  /** Display-only name; `modelId` stays the raw value used for selection */
                  ...
                  return (
                    <MenuItem
                      ...
                      className="flex h-[34px] w-full cursor-pointer items-center justify-start rounded-[7px] pl-5 text-[12.5px] leading-[18.75px]"
                    >
                      <div className="flex min-w-0 items-center gap-2">
                        ...
                        <span className="truncate text-text-primary">{modelName}</span>
                      </div>
                      {isGlobal && (
                        <EarthIcon className="ml-auto size-3.5 text-brand-purple" aria-hidden="true" />
                      )}
                      {isModelSelected && (
                        <>
                          <CheckCircle2
            ...
            return (
              <MenuItem
                ...
                className="flex h-9 w-full cursor-pointer items-center justify-between text-[13px] leading-[19.5px]"
              >
                <div className="flex min-w-0 items-center gap-2">
                  <span className="truncate text-text-primary">{endpoint.label}</span>
                </div>
                {isEndpointSelected && (
                  <>
                    <CheckCircle2
