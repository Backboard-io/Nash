...
export const tierOptions: Array<{ value: TModelTier; label: string; description: string }> = [
  { value: 'free', label: 'Free', description: 'Free / open-weight models' },
    ...
    if (item.models && item.models.length > 0) {
      return item.models.some((modelId) => {
        if (
          ...
          formatModelName(modelId.name).toLowerCase().includes(searchTermLower)
        ) {
          return true;
        }
...
export function filterModels(
  ...
  return models.filter((modelId) => {
    ...
    return (
      ...
      formatModelName(modelName).toLowerCase().includes(searchTermLower)
    );
  });
}
...
export const getDisplayValue = ({
  ...
  if (selectedValues.modelSpec) {
    ...
    return spec?.label || formatModelName(spec?.name) || localize('com_ui_select_model');
  }
  ...
  if (selectedValues.model && selectedValues.endpoint) {
    ...
    return formatModelName(selectedValues.model);
  }
