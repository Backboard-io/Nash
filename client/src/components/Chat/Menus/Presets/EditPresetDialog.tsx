...
import { QueryKeys, getModelName, isAgentsEndpoint } from 'librechat-data-provider';
import {
  Input,
  Label,
  OGDialog,
...
const EditPresetDialog = ({
  ...
  useEffect(() => {
    ...
    const models = modelsConfig[presetEndpoint]?.map(getModelName);
    if (!models) {
      return;
    }
    if (!models.length) {
        ...
        <div className="flex w-full flex-col gap-2 px-1 pb-4 md:gap-4">
          ...
          <div className="flex justify-end gap-2 border-t border-border-medium pt-2 md:pt-4">
            ...
            <button
              ...
              className="rounded-lg bg-brand-purple px-3 py-2 text-sm font-medium text-white hover:bg-brand-purple-hover focus:outline-none focus:ring-2 focus:ring-brand-purple focus:ring-offset-2 md:px-4"
            >
