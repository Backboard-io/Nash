  ...
  return (
    <div className="relative flex h-full w-full flex-col items-center overflow-y-auto">
      <div className="w-full max-w-3xl px-6 pb-8 pt-20">
        ...
        {/* -mx-2 cancels the page's gutter so ChatForm's own max-w-[736px]
            applies in full — otherwise the card lands 16px narrower here than
            the composer on a new chat. */}
        <div className="-mx-2 mb-6">
          <ChatForm index={index} />
          {/* The caveat follows the composer here too. */}
          <ComposerDisclaimer />
        </div>

        <div className="flex flex-col">
          {/* Same section label as the sidebar: 11px, 0.06em, tertiary. */}
          <h2 className="mb-2 px-3 text-[11px] font-medium uppercase leading-[16.5px] tracking-[0.06em] text-text-tertiary">
          ...
          {isLoading ? (
            ...
            /* No dividers — the row's own hover fill and the 2px rhythm carry
               the separation, as everywhere else in the app. */
            <div className="flex flex-col gap-[2px]">
              {conversations.map((convo) => (
                <div
                  key={convo.conversationId as string}
                  className="group relative flex w-full items-center rounded-[10px] transition-colors hover:bg-surface-hover"
                >
                  <button
                    type="button"
                    onClick={() => handleConvoClick(convo as TConversation)}
                    className="flex min-w-0 flex-1 items-center gap-4 px-3 py-[13px] text-left"
                  >
                    <span className="min-w-0 flex-1 truncate text-[13.5px] font-medium leading-[20px] text-text-primary">
                      {(convo.title as string) || localize('com_ui_new_chat')}
                    </span>
                    <span className="shrink-0 text-[11.5px] leading-[17px] text-text-tertiary">
                      {formatRelativeDate(
                  ...
                  <button
                    ...
                    className="mr-2 grid h-[26px] w-[26px] shrink-0 place-items-center rounded-[7px] text-text-tertiary opacity-0 transition-all hover:bg-surface-active hover:text-text-primary focus-visible:opacity-100 group-hover:opacity-100"
