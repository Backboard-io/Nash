...
const DownloadableFileCard = memo(
  ({
    ...
    return (
      <span
        className={cn(
          'my-1 inline-flex w-full max-w-full items-center gap-2.5 rounded-[13px] bg-surface-chat px-3.5 py-3 align-middle text-sm text-text-primary',
          // §9 `dl` is an icon tile, a name, meta and a Download button — the
          // fill is what makes it a card, so no outline on top of it. The
          // failed state keeps one, because §1 reserves red for a failure that
          // has already happened and the fill alone cannot say that.
          status === 'error' && 'border border-border-destructive',
          className,
        )}
      >
