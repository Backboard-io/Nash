...
export const ThinkingContent: FC<{
  ...
  return (
    <div className="relative rounded-3xl border border-border-medium bg-surface-tertiary p-4 pb-10 text-text-secondary">
      {/* leading-relaxed (1.625), not a fixed 26px: this text follows the font-size
          setting, and a pinned line-height would read airier as the text shrinks.
          1.625 is exactly 26/16, so it is unchanged at the previous size. */}
      <p className={cn('whitespace-pre-wrap leading-relaxed', fontSize)}>{children}</p>
    </div>
  );
});
      ...
      <div className="group/thinking flex w-full items-center justify-between gap-2">
        <button
          ...
          className={cn(
            ...
            fontSize,
            isThinking ? 'text-brand-purple' : 'text-text-primary',
          )}
        >
          <ChevronDown
            className={cn(
              'icon-sm mr-1.5 transform-gpu transition-transform duration-swap',
              isThinking ? 'text-brand-purple' : 'text-text-secondary',
        ...
        {isThinking ? (
          <span className="mr-1 inline-flex items-center gap-1" aria-hidden="true">
            <span
              className="h-1.5 w-1.5 animate-pulse rounded-full bg-brand-purple"
...
export const FloatingThinkingBar = memo(
  ({
    ...
    return (
      <div
        className={cn(
          'absolute bottom-3 right-3 flex items-center gap-2 transition-opacity duration-hover',
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn('grid transition-all duration-swap ease-nash', isExpanded && 'mb-8')}
        style={{
