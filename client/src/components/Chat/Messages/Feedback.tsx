...
function FeedbackOptionButton({
  ...
  return (
    <button
      className={cn(
        'flex w-full items-center gap-3 rounded-xl p-2 text-text-secondary transition-colors duration-hover hover:bg-surface-hover hover:text-text-primary',
      ...
      aria-pressed={active}
    >
      {/* 16: §12 puts a dialog row at 16–17. 19 is the top-bar/tile band,
          which made a reason in this list heavier than the message actions
          that opened it. */}
      <Icon size="16" bold={active} aria-hidden="true" />
      <span>{label}</span>
    </button>
  );
}
      ...
      <Ariakit.PopoverAnchor
        ...
        render={
          <button
            ...
            aria-haspopup="menu"
          >
            <ThumbUpIcon size="15" bold={feedback?.rating === 'thumbsUp'} />
          </button>
        }
      />
      <Ariakit.Popover
      ...
      <Ariakit.PopoverAnchor
        ...
        render={
          <button
            ...
            aria-haspopup="menu"
          >
            <ThumbDownIcon size="15" bold={feedback?.rating === 'thumbsDown'} />
          </button>
        }
      />
      <Ariakit.Popover
  ...
  const renderSingleFeedbackButton = () => {
    ...
    return (
      <button
        ...
        aria-pressed="true"
      >
        {/* 15, like every other glyph in the action row — this one state
            rendered a pixel larger than the buttons beside it. */}
        <Icon size="15" bold />
      </button>
    );
  };
