...
function VoiceMode({ open, conversationId, model, onClose, onConversationId }: Props) {
  ...
  return createPortal(
    <div
      ...
      aria-modal="true"
    >
      <div className="absolute left-4 top-4 rounded-full border border-brand-purple/40 bg-brand-purple/10 px-2 py-0.5 text-xs font-medium text-text-accent">
      ...
      <button
        type="button"
        onClick={onClose}
        className="mt-8 inline-flex items-center gap-2 rounded-full border border-brand-purple/40 bg-brand-purple/10 px-5 py-2.5 text-sm font-medium text-text-accent transition-colors hover:bg-brand-purple/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
