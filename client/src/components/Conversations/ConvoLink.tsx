...
import { truncateChatTitle } from '~/utils/chatTitle';
...
const ConvoLink: React.FC<ConvoLinkProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Three words on screen; the full title stays on `title` and
            `aria-label` above, so hover and screen readers still get all of
            it. */}
        {title ? truncateChatTitle(title) : localize('com_ui_untitled')}
      </div>
    </div>
  );
};
