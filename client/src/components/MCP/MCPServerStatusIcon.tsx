  ...
  if (canCancel) {
    return (
      <button
        type="button"
        onClick={onCancel}
        className="group flex size-6 items-center justify-center rounded p-1 hover:bg-red-100 max-sm:-mx-1 max-sm:-my-2 max-sm:h-10 max-sm:w-8 dark:hover:bg-red-900/20"
...
/**
 * Fix — shown for disconnected/error states.
 *
 * A labelled button, not a bare plug glyph. This is the one row in the list
 * that needs an action taken before it will work, and an icon on its own left
 * the reader to guess both what is wrong and what pressing it does; the row
 * now says "Needs auth" and the button says "Fix".
 */
function ConnectButton({ serverName, onConfigClick }: StatusIconProps) {
  return (
    <button
      ...
      className="inline-flex h-[30px] shrink-0 items-center gap-[6px] rounded-[8px] bg-surface-active px-[11px] text-[12.5px] font-medium text-text-primary transition-colors hover:bg-surface-hover-alt focus:outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-border-heavy"
...
function ConfigureButton({ serverName, onConfigClick }: StatusIconProps) {
  return (
    <button
      type="button"
      onClick={onConfigClick}
      className="flex size-6 items-center justify-center rounded p-1 hover:bg-surface-secondary max-sm:-mx-1 max-sm:-my-2 max-sm:h-10 max-sm:w-8"
