...
export default function MCPConfigDialog({
  ...
  const renderStatusBadge = () => {
    ...
    if (connectionState === 'connected') {
      return (
        <div className="flex items-center gap-2 rounded-full bg-violet-50 px-2 py-0.5 text-xs font-medium text-brand-purple dark:bg-violet-950 dark:text-violet-400">
  ...
  return (
    <OGDialog open={isOpen} onOpenChange={onOpenChange}>
      <OGDialogContent
        className="flex max-h-[calc(100vh-2rem)] w-11/12 max-w-lg flex-col space-y-2"
        title={fullTitle}
      >
        <OGDialogHeader>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            <OGDialogTitle className="min-w-0 break-words text-xl">
