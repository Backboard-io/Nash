...
import { useLocalize, useCopyToClipboard, useMCPServerManager } from '~/hooks';
...
export default function MCPServerDialog({
  ...
  const formHook = useMCPServerForm({
    server,
    onSuccess: (serverName, result) => {
      if (result.isOAuth && result.isNew && result.hasManualClient) {
        // Pre-registered client: the user still has to add our redirect URI
        // to their OAuth app, so show it before any connect attempt.
      ...
      if (result.isOAuth && result.isNew) {
        // Fully automatic (discovery + dynamic registration): kick off the
        // connect right away — opens the authorization popup and polls.
