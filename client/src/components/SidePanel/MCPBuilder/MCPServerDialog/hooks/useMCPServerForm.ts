...
export interface MCPServerFormSuccess {
  /** The saved server is OAuth-authenticated (chosen in the form, or
   *  auto-detected by the backend on a 401 from a bare-URL add). */
  isOAuth: boolean;
  /** This submit created the server (vs. edited an existing one). */
  isNew: boolean;
  /** The user pasted a pre-registered client ID, so they still need the
   *  redirect URI to finish setup on the provider's side. */
  hasManualClient: boolean;
}

interface UseMCPServerFormProps {
  server?: MCPServerDefinition | null;
  onSuccess?: (serverName: string, result: MCPServerFormSuccess) => void;
  onClose?: () => void;
}
...
export function useMCPServerForm({ server, onSuccess, onClose }: UseMCPServerFormProps) {
  ...
  const onSubmit = methods.handleSubmit(async (formData: MCPServerFormData) => {
    ...
    try {
      ...
      // Add OAuth configuration. All fields are optional: an empty block is
      // sent as {auto: true} and the backend resolves everything via MCP-spec
      ...
      // OAuth either chosen in the form, or auto-detected by the backend when
      // a bare-URL server answered 401 and OAuth discovery succeeded.
      const isOAuth = formData.auth.auth_type === AuthTypeEnum.OAuth || Boolean(result.oauth);
      onSuccess?.(result.serverName, {
        isOAuth,
        isNew: !server,
        hasManualClient: Boolean(formData.auth.oauth_client_id?.trim()),
      });
    } catch (error: unknown) {
