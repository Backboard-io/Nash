...
import type { CreateMemoryParams, CreateMemoryResponse } from '~/data-provider/Memories/queries';
...
function parseImportedMemories(text: string): CreateMemoryParams[] {
  const lines = text.split('\n').filter((line) => line.trim().length > 0);
  const memories: CreateMemoryParams[] = [];
...
export default function MemoryImportDialog({
  ...
  const { mutate: createMemory } = useCreateMemoryMutation({
  ...
  const handleImport = useCallback(() => {
    ...
    const createNext = (
      index: number,
      mutate: UseMutateFunction<CreateMemoryResponse, Error, CreateMemoryParams>,
    ) => {
  ...
  const stepOneContent = (
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        >
          {copied ? (
            <Check className="size-3.5 text-brand-purple" aria-hidden="true" />
          ) : (
      ...
      <p className="text-xs text-text-tertiary">
        {localize('com_ui_memory_import_step1_hint')}
      </p>
    </div>
  );
  ...
  return (
    <OGDialog open={open} onOpenChange={handleOpenChange}>
      {children}
      <OGDialogTemplate
        title={localize('com_ui_memory_import_title')}
