...
import useResetAgentForm from '~/hooks/Agents/useResetAgentForm';
...
export default function AgentPanel() {
  ...
  const resetAgentForm = useResetAgentForm(reset);
  ...
  // Populate the builder when an existing persona is opened for editing. The
  // panel no longer renders the AgentSelect dropdown that used to own this reset,
  // so without it the form stays empty and the panel reads "Create Persona".
  useEffect(() => {
        ...
        {activePanel === Panel.builder && (
          <div className="mx-2 mb-4 mt-2">
            <h2 className="text-xl font-semibold text-text-primary">
              {agent_id ? localize('com_ui_edit_persona') : localize('com_ui_create_persona')}
            </h2>
            <p className="mt-1 text-sm text-text-secondary">
              {agent_id
                ? localize('com_ui_edit_persona_subtitle')
                : localize('com_ui_create_persona_subtitle')}
            </p>
          </div>
        )}
