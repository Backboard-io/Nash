...
function getToolItemStyle(isDeferred: boolean, isProgrammatic: boolean): string {
  ...
  if (isProgrammatic) {
    return 'border-brand-purple/50 bg-brand-purple/5 hover:bg-brand-purple-hover/10';
  }
  ...
  return (
    ...
    >
      <Checkbox
        ...
        onClick={(e) => e.stopPropagation()}
        className="relative mr-2 inline-flex h-4 w-4 shrink-0 cursor-pointer rounded border border-border-medium transition-[border-color] duration-hover hover:border-border-heavy focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background"
      ...
      <div className="ml-2 flex shrink-0 items-center gap-1.5">
        ...
        {isProgrammatic && <Code2 className="h-3.5 w-3.5 text-brand-purple" aria-hidden="true" />}
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              className={cn(
                'flex h-6 w-6 items-center justify-center rounded transition-all duration-hover',
