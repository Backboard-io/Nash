...
export const Banner = ({ onHeightChange }: { onHeightChange?: (height: number) => void }) => {
  ...
  useEffect(() => {
    ...
    // Mobile banners wrap (max-sm:whitespace-normal), so the height changes
    // with viewport width — without this, Root keeps subtracting a stale height.
  ...
  return (
    ...
    >
      <div
        className={cn(
          'text-md w-full truncate text-center max-sm:whitespace-normal max-sm:break-words [&_a]:text-blue-700 [&_a]:underline dark:[&_a]:text-blue-400',
