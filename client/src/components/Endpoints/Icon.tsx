...
const UserAvatar = memo(({ size, user, avatarSrc, username, className }: UserAvatarProps) => {
  ...
  const renderDefaultAvatar = () => (
    <div
      ...
      className="relative flex h-9 w-9 items-center justify-center rounded-[7px] p-1 text-white"
    >
      <UserIcon />
    </div>
  );
...
const Icon: React.FC<IconProps> = memo((props) => {
  ...
  if (isCreatedByUser) {
    const username = user?.nickname?.trim() || user?.name || user?.username || localize('com_nav_user');
    return (
      <UserAvatar
        size={size}
        user={user}
