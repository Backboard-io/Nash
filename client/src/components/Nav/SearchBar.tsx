...
const SearchBar = forwardRef((props: SearchBarProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  return (
    <div
      ref={ref}
      className="group relative flex h-9 flex-shrink-0 items-center gap-[9px] rounded-[9px] border border-border-light bg-[#F6F6F7] px-[11px] text-text-primary transition-colors focus-within:border-ring-primary dark:bg-surface-tertiary-alt"
    >
      <Search aria-hidden="true" size={14} className="flex-shrink-0 text-text-secondary" />
      <input
        type="text"
        ref={inputRef}
        className="m-0 mr-0 w-full border-none bg-transparent p-0 text-[13px] leading-[19.5px] placeholder:text-text-secondary-alt focus-visible:outline-none"
        value={text}
        onChange={onChange}
        onKeyDown={(e) => {
      ...
      <button
        ...
        className={cn(
          'absolute right-[9px] flex h-5 w-5 items-center justify-center rounded-full border-none bg-transparent p-0 transition-opacity duration-200',
