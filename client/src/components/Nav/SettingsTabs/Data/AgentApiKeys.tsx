      ...
      <OGDialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-4">
          {!newKey ? (
            ...
            <>
              <div className="rounded-lg bg-surface-warning-subtle p-4">
...
function KeyItem({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border-light p-3">
      ...
      <div>
        {confirmDelete ? (
          ...
          >
            <Trash2 className="h-4 w-4 text-text-secondary hover:text-text-destructive" />
          </Button>
        )}
      </div>
    </div>
...
function ApiKeysContent({ isOpen }: { isOpen: boolean }) {
  ...
  if (error) {
    return <div className="text-sm text-text-destructive">{localize('com_ui_api_keys_load_error')}</div>;
  }
