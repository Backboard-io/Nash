...
function Preferences() {
  ...
  return (
    /* No `border-t` here — General already puts a `divide-y` between every
       row, so this drew a second hairline a few pixels under the first. */
    <div className="flex flex-col gap-4">
      {/* The group's heading sits further from its first row than the rows sit
          from each other, so it reads as a heading rather than a fourth row. */}
      <div className="-mb-0.5 text-[13.5px] font-medium leading-[20px] text-text-primary">
        Defaults
      </div>
