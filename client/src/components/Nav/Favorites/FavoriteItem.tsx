...
import { renderAgentAvatar, cn, formatModelName } from '~/utils';
...
export default function FavoriteItem({
  ...
  const getName = (): string => {
    ...
    return formatModelName((item as FavoriteModel).model);
  };
      ...
      >
        <DropdownPopup
          ...
          trigger={
            <Menu.MenuButton
              className={cn(
                'inline-flex h-7 w-7 items-center justify-center rounded-lg border-none p-0 text-sm font-medium ring-ring-primary transition-all duration-hover ease-nash focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:opacity-50',
                isPopoverActive
                  ? 'opacity-100'
