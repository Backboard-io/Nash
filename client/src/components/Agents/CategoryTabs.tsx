...
const CategoryTabs: React.FC<CategoryTabsProps> = ({
  ...
  /* §5: a filter strip is tabs, not a segmented control. The container had its
     own fill with the selected tab lifted inside it, which reads as one control
     with a moving thumb; these are peers, and only the chosen one wears a chip.
     Same FilterPill the bookmark kinds and memory scopes use, so the three
     strips cannot drift. */
  const tabsContent = (
    <div
      className={cn(
    ...
    >
      {categories.map((category, index) => (
        <FilterPill
          ...
          /* Numbered like the bookmark kinds, so a segment says how much is
             behind it before you switch to it. Undefined rather than 0 when a
             count is not known yet — FilterPill hides the number, where a
             literal "0" would claim the tab is empty. */
          count={category.count != null && category.count > 0 ? category.count : undefined}
