...
interface AgentDetailContentProps {
  ...
  onStartChat?: () => void;
  /**
   * Install a catalogue persona. Passed for explore personas so the dialog
   * leads with Install too — otherwise opening a template and pressing "Start
   * chat" walks straight past the install the card is asking for, into a chat
   * with an agent the account does not own.
   */
  onInstall?: (agent: t.Agent) => void;
  /** The user's copy of this template, once installed. Chat targets this. */
...
const AgentDetailContent: React.FC<AgentDetailContentProps> = ({
  ...
  const handleStartChat = () => {
    /* Explore personas chat as the user's installed copy, never as the
       ownerless catalogue template. */
    const target = variant === 'explore' ? (installedAgent ?? agent) : agent;
    if (target) {
      ...
      if (listResp != null) {
        if (!listResp.data.some((a) => a.id === target.id)) {
          const currentAgents = [target, ...JSON.parse(JSON.stringify(listResp.data))];
          queryClient.setQueryData<AgentListResponse>(keys, { ...listResp, data: currentAgents });
        }
      }

      localStorage.setItem(`${LocalStorageKeys.AGENT_ID_PREFIX}0`, target.id);
      ...
      const template = {
        ...
        agent_id: target.id,
        title: localize('com_agents_chat_with', { name: target.name || localize('com_ui_agent') }),
      };
