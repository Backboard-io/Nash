...
describe('Accessibility Improvements', () => {
  ...
  describe('Focus Management', () => {
    it('keeps the search field keyboard reachable', () => {
      const { container } = render(<SearchBar value="" onSearch={jest.fn()} />);
      const searchInput = container.querySelector('input');

      /* This used to assert a `focus:` ring. The field no longer draws one —
       * a product decision, recorded here rather than asserted away: §6 gives
       * the focus treatment to `.inp`, and search fields were stripped of the
       * accent ring deliberately. What still has to hold is that the field is
       * reachable and labelled, so that is what this checks. */
