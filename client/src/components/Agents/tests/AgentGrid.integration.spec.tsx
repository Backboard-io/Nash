...
jest.mock('~/data-provider/Agents', () => ({
  ...
  useDeleteAgentMutation: jest.fn(() => ({ mutate: jest.fn() })),
}));
...
// Mock SmartLoader
// Mirrors the real useHasData contract rather than hardcoding `true`: the grid
// now only shows a skeleton on a first load with nothing to display, so a mock
// that always claims data exists suppresses the loading state under test.
jest.mock('../SmartLoader', () => ({
  useHasData: jest.fn((data?: { data?: unknown[] }) => (data?.data?.length ?? 0) > 0),
}));
...
describe('AgentGrid Integration with useGetMarketplaceAgentsQuery', () => {
  ...
  describe('Query Integration', () => {
    ...
    it('should call useGetMarketplaceAgentsQuery with promoted=1 for promoted category', () => {
      ...
      render(
        <Wrapper>
          <AgentGrid category="promoted" searchQuery="" onSelectAgent={mockOnSelectAgent} />
        </Wrapper>,
      );
    ...
    it('should call useGetMarketplaceAgentsQuery without category filter for "all" category', () => {
      ...
      render(
        <Wrapper>
          <AgentGrid category="all" searchQuery="" onSelectAgent={mockOnSelectAgent} />
        </Wrapper>,
      );
    ...
    it('should not include category in search when category is "all" or "promoted"', () => {
      ...
      render(
        <Wrapper>
          <AgentGrid category="all" searchQuery="test" onSelectAgent={mockOnSelectAgent} />
        </Wrapper>,
      );
  ...
  describe('Loading States', () => {
    it('should show loading state when isLoading is true', () => {
      ...
      // Should show the loading skeleton, exposed as a status region
  ...
  describe('Infinite Scroll Functionality', () => {
    ...
    it('should render agents without a loading indicator when hasNextPage is false and agents exist', () => {
      // The teal redesign removed the explicit "end of results" text message.
      // When pagination is exhausted and agents exist, the grid simply renders
      // the cards with no trailing loading spinner.
      mockUseMarketplaceAgentsInfiniteQuery.mockReturnValue({
        ...defaultMockQueryResult,
        hasNextPage: false,
        isFetchingNextPage: false,
