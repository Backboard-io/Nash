...
// Per-conversation TTS voice override for Voice Mode. Empty string means
// "use the user default" (store.defaultTTSVoice) which in turn falls back to
// the server default. Keyed by conversationId so it stays scoped to one chat,
// distinct from the user-wide Settings → Preferences default.
const ttsVoiceByConversation = atomFamily<string, string | number>({
  key: 'ttsVoiceByConversation',
  default: '',
});

// Per-conversation image-generation model override. Empty string means "use
// the user default" (store.defaultImageModel) which in turn falls back to the
// deploy default. Keyed by conversationId so the composer's per-chat picker
// stays scoped to one chat, distinct from the Settings → Preferences default.
const imageModelByConversation = atomFamily<string, string | number>({
  key: 'imageModelByConversation',
  default: '',
});
...
// Per-conversation buffer of large pasted blocks lifted out of the textarea and
// shown as "Pasted code/text" cards in the composer. Injected into the message
// text on submit, then cleared. Keyed by conversationId (NEW_CONVO before the
// first send), mirroring imageModelByConversation.
const pastedBlocksByConversation = atomFamily<PastedBlock[], string | number>({
  key: 'pastedBlocksByConversation',
  default: [],
});
...
export default {
  ...
  ttsVoiceByConversation,
  imageModelByConversation,
  pastedBlocksByConversation,
