...
export const useContinueSharedConvoMutation = (
  options?: t.MutationOptions<t.TForkConvoResponse, string>,
): UseMutationResult<t.TForkConvoResponse, unknown, string, unknown> => {
  const queryClient = useQueryClient();
  const { onSuccess, ..._options } = options || {};

  return useMutation((shareId: string) => dataService.continueSharedConvo(shareId), {
    onSuccess: (data, vars, context) => {
      ...
      // Seed the caches so /c/<newId> hydrates instantly when we navigate there
      // with plain react-router (the share page can't use navigateToConvo — it
      // lives outside the chat providers and lacks the endpoints/models config).
      queryClient.setQueryData([QueryKeys.conversation, newConversationId], newConversation);
      addConvoToAllQueries(queryClient, newConversation);
      queryClient.setQueryData([QueryKeys.messages, newConversationId], data.messages);
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.allConversations],
        refetchPage: (_, index) => index === 0,
      });
