...
export const useUploadFileMutation = (
  ...
  /**
   * Per-file upload progress, 0..1, keyed by the temp file id the caller put
   * on the FormData. Only fires while the browser reports total bytes.
   */
...
> => {
  ...
  return useMutation([MutationKeys.fileUpload], {
    mutationFn: (body: FormData) => {
      ...
      const report =
        onProgress && fileId !== ''
          ? (progress: number) => onProgress(fileId, progress)
          : undefined;
      if (isAssistantsEndpoint(endpoint) && version === '2') {
        return dataService.uploadFile(body, signal, report);
      }

      if (width !== '' && height !== '') {
        return dataService.uploadImage(body, signal, report);
      }
