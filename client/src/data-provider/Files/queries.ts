...
/**
 * Aggregate storage usage for the Library rail. Computed server-side from the
 * same deduplicated file list the table renders, so the cards, the total and
 * the meter cannot disagree.
 */
export const useGetFilesUsage = <TData = t.TFileUsage>(
  config?: UseQueryOptions<t.TFileUsage, unknown, TData>,
): QueryObserverResult<TData, unknown> => {
  const queriesEnabled = useRecoilValue<boolean>(store.queriesEnabled);
  return useQuery<t.TFileUsage, unknown, TData>(
    [QueryKeys.filesUsage],
    () => dataService.getFilesUsage(),
    {
      refetchOnWindowFocus: false,
      refetchOnReconnect: false,
      ...config,
