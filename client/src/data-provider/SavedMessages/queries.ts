...
export const useSavedMessagesQuery = (
  ...
  return useQuery<TSavedMessagesResponse>(
    ...
    // The API can answer with an error object instead of an array; normalize
    // here so consumers can trust the TSavedMessagesResponse type.
    async () => {
      const data = await dataService.getSavedMessages(folderId);
      return Array.isArray(data) ? data : [];
    },
    {
      refetchOnWindowFocus: false,
      refetchOnReconnect: false,
      /* Deliberately NOT `refetchOnMount: false`.
       *
       * Invalidating a query only refetches it while something is mounted and
       * watching. Bookmarking from a chat invalidates this list, but the
       * Bookmarks page is not open, so it is merely marked stale — and with
       * refetch-on-mount off it stayed stale when the page did open. The new
       * bookmark appeared only after a hard reload. Refetching a stale list on
       * mount is what makes the invalidation mean anything. */
      ...config,
    },
  );
};
