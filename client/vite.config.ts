...
export default defineConfig(({ command }) => ({
  ...
  server: {
    ...
    proxy: {
      '/api': {
        target: backendURL,
        changeOrigin: true,
        // Forward the WebSocket upgrade for `/api/voice/realtime` to Flask.
        // Without this, the browser hangs on the handshake and times out.
        ws: true,
        configure: (proxy) => {
  ...
  // Only `VITE_*` is exposed to the client bundle. Server-side env vars
  // (DOMAIN_*, ALLOW_*, SCRIPT_*, secrets, etc.) must NOT be baked into the
  // browser bundle — keep them on the Flask backend only.
  envDir: '../',
  envPrefix: ['VITE_'],
  plugins: [
    react(),
